...
function SectionHeader({ num, eyebrow, title }: { num: number; eyebrow: string; title: string }) {
  return (
    <div className="mb-10 flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-10">
      <div className="flex h-[4.75rem] w-[4.75rem] shrink-0 skew-y-[-4deg] items-center justify-center rounded-3xl border-2 border-[#E9C349]/55 bg-[#E9C349]/14 text-2xl font-black text-[#F6DA87] shadow-[0_0_40px_rgba(233,195,73,0.22)]">
        ...
        <ul className="mt-4 grid gap-3 text-sm leading-7 text-white/66">
          {flow.valorEstrategico.map((line, i) => (
            <li key={i} className="flex gap-3">
              <span className="mt-2 inline-block h-1 w-8 shrink-0 rounded-full bg-gradient-to-r from-[#F6DA87] to-[#E9C349]" />
              <span>{line}</span>
            </li>
          ))}
        </ul>
          ...
          <section id={id} className="scroll-mt-36">
            ...
            <div className="mt-12 space-y-8">
              {s.bullets.map((bullet, idx) =>
                idx === 0 ? (
                  <p key={idx} className="border-l-[3px] border-[#E9C349] pl-6 text-lg font-semibold leading-8 text-white/76">
                    {bullet}
                  </p>
                ) : (
            ...
            <ul className="mt-10 grid gap-5">
              {s.bullets.map((bullet, idx) => (
                <motion.li
                  ...
                  whileInView="visible"
                  viewport={{ once: true }}
                  className="rounded-[1.6rem] border border-[#E9C349]/28 bg-[linear-gradient(120deg,rgba(233,195,73,0.11),transparent_55%)] p-7 text-sm leading-7 text-white/70 shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-2xl"
                >
                  {bullet}
                </motion.li>
              ))}
              ...
              {s.tiers.map((t, i) => (
                <motion.article
                  ...
                  className={`rounded-[2rem] border p-8 backdrop-blur-2xl ${
                    i === s.tiers.length - 1
                      ? 'border-[#E9C349]/45 bg-[#18140B]/90 shadow-[0_0_80px_rgba(233,195,73,0.14)] md:col-span-2 md:flex md:flex-row md:items-center md:gap-14'
                      : 'border-white/[0.09] bg-[#101010]/80'
                  }`}
                >
                  <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl border border-[#E9C349]/38 bg-[#E9C349]/13 text-[#F6DA87] md:mb-0">
                    {(i + 1).toString()}
                  </div>
                  <div>
                  ...
                  <div key={gi}>
                    ...
                    <ul className="grid gap-4">
                      {g.items.map((item, ii) => (
                        <li
                          key={ii}
                          className="relative rounded-2xl border border-white/[0.07] bg-[#121212]/70 py-5 pl-[1.85rem] pr-6 text-sm leading-7 text-white/64 backdrop-blur-xl before:absolute before:left-6 before:top-6 before:h-2 before:w-2 before:-translate-x-px before:rounded-full before:bg-[#E9C349]"
                        >
                          {item}
                        </li>
                      ))}
...
export default function ExecutiveSummaryLanding() {
  ...
  return (
    <div
      className={`${inter.variable} min-h-screen bg-[#030303] text-white antialiased selection:bg-[#E9C349] selection:text-black`}
    ...
    >
      ...
      <div className="pointer-events-none fixed inset-0 -z-20 opacity-[0.45] bg-[radial-gradient(ellipse_90%_50%_at_50%_-10%,rgba(233,195,73,0.22),transparent_55%),radial-gradient(ellipse_60%_40%_at_100%_75%,rgba(246,218,135,0.08),transparent_50%)]" />
        ...
        <div className="mx-auto flex h-[4.05rem] max-w-[1480px] items-center justify-between px-6 sm:px-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              className="rounded-full bg-gradient-to-r from-[#F6DA87] via-[#E9C349] to-[#A87B1F] px-5 py-2.5 text-[10px] font-black uppercase tracking-[0.2em] text-black shadow-[0_0_24px_rgba(233,195,73,0.28)]"
            >
              {c.navWaitlist}
            </a>
          </div>
      ...
      <aside className="fixed bottom-8 right-5 z-40 hidden flex-col items-end gap-3 xl:flex" aria-label={c.tocLabel}>
        <AnimatePresence initial={false}>
          {tocOpen ? (
            <motion.div
              ...
              className="w-[min(18rem,calc(100vw-2.5rem))] max-h-[min(56vh,28rem)] overflow-hidden rounded-[1.85rem] border border-[#E9C349]/25 bg-[#070707]/95 shadow-[0_34px_100px_rgba(0,0,0,0.65),0_0_0_1px_rgba(233,195,73,0.08)] backdrop-blur-2xl"
            >
              <div className="flex items-center justify-between gap-3 border-b border-white/[0.06] px-4 py-3">
                ...
                <button
                  ...
                  className="rounded-full border border-white/12 bg-white/[0.04] px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-white/55 transition hover:border-[#E9C349]/45 hover:text-[#F6DA87]"
                >
                  {c.tocPanelDismiss}
                </button>
              </div>
                  ...
                  {toc.map((item) =>
                    item.divider ? (
                      <li key={item.id} className="pb-1 pt-2">
                        <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-[#E9C349]/55">{item.label}</span>
                      </li>
                    ) : (
                      <li key={item.id}>
                        <a
        ...
        <button
          ...
          className="group relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full border-2 border-[#E9C349]/45 bg-[linear-gradient(145deg,rgba(233,195,73,0.22),rgba(8,8,8,0.96))] text-[#F6DA87] shadow-[0_0_32px_rgba(233,195,73,0.22),inset_0_1px_0_rgba(255,255,255,0.12)] transition hover:border-[#F6DA87]/55 hover:shadow-[0_0_44px_rgba(233,195,73,0.35)]"
      ...
      <main className="relative mx-auto w-full max-w-[1180px] px-6 pb-40 pt-[7.75rem] sm:px-10">
        ...
        >
          <div className="absolute -left-[14%] -top-[18%] h-[22rem] w-[22rem] rounded-full bg-[#E9C349]/07 blur-[100px]" />
          <div className="absolute -bottom-[30%] -right-[8%] h-[18rem] w-[18rem] rounded-full bg-[#246]/10 blur-[90px]" />
          <div className="relative overflow-hidden rounded-[2.85rem] border border-[#E9C349]/35 bg-[#080808]/90 p-10 shadow-[0_0_120px_rgba(233,195,73,0.12),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-3xl sm:p-14 lg:p-16">
            <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(120deg,rgba(233,195,73,0.16)_0%,transparent_38%,transparent_62%,rgba(246,218,135,0.06)_100%)]" />
            ...
            <p className="relative mt-10 max-w-3xl border-l-[3px] border-[#E9C349] pl-7 text-xl font-semibold leading-8 text-white/78">
              {c.headerSubtitle}
            </p>
            <div className="relative mt-12 flex flex-wrap items-center gap-4">
              ...
              <div className="h-px flex-1 min-w-[3rem] bg-gradient-to-r from-[#E9C349]/50 to-transparent" />
            </div>
          </div>
        </motion.header>
        ...
        <div className="relative mb-36 overflow-hidden rounded-[2.75rem] border border-white/[0.07] bg-[#090909]/60 p-10 backdrop-blur-2xl sm:p-14">
          <Reveal>
            <p className="text-center text-xs font-black uppercase tracking-[0.34em] text-[#E9C349]/90">{c.blueprintLabel}</p>
            <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {c.blueprintMotors.map(([code, word], idx) => (
                <motion.div
                  ...
                  whileInView="visible"
                  viewport={{ once: true }}
                  className={`flex items-center gap-6 rounded-[1.85rem] border px-7 py-6 transition duration-300 hover:border-[#E9C349]/45 ${
                    idx === 0 ? 'border-[#E9C349]/42 bg-[#E9C349]/07' : 'border-white/[0.08] bg-[#0c0c0c]/80'
                  }`}
                >
                  <span className="grid h-12 w-12 place-items-center rounded-2xl border border-[#E9C349]/35 bg-black/40 font-mono text-sm font-black text-[#F6DA87]">{code}</span>
                  <div>
            ...
            <motion.section key={block.num} variants={reveal} id={`s${block.num}`} className="scroll-mt-36">
              ...
              <div className="mt-14 grid gap-6 lg:grid-cols-[1fr,1fr]">
                ...
                <div className="flex flex-col gap-6">
                  <div className="flex-1 rounded-[1.85rem] border border-[#E9C349]/30 bg-[#18140e]/74 p-7 backdrop-blur-2xl sm:p-8">
                    ...
                    <ul className="mt-5 grid gap-4">
                      {block.valorBullets.map((v, vi) => (
                        <li key={vi} className="flex gap-3 text-sm leading-7 text-white/67">
                          <span className="font-black text-[#E9C349]">{vi + 1}.</span>
                          {v}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div className="rounded-[1.85rem] border border-white/[0.08] bg-[#090909]/80 p-7 shadow-[inset_0_0_0_1px_rgba(233,195,73,0.08)] backdrop-blur-2xl sm:p-8">
