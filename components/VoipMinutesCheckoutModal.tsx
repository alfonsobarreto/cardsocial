/**
 * Compra contextual de minutos AirTime (VoIP / Ghost-Link).
 */
...
import { getCommerceConfig, type CommerceVoipMinutePack } from '@/services/commerceConfigService';
import { useCoreT } from '@/services/coreI18n';
import { intlLocaleTagForAppLanguage, useLanguage } from '@/services/language';
import { useLookMode } from '@/services/lookMode';
import { redeemVoipMinutePack } from '@/services/qrApi';
...
export function VoipMinutesCheckoutModal({ visible, onClose }: Props) {
  const t = useCoreT();
  ...
  const [packs, setPacks] = useState<CommerceVoipMinutePack[]>([]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (res.ok) {
          setIssue('none');
          setPacks(res.data.voipMinutePacks);
        } else {
          setPacks([]);
  ...
  const handleBuy = async (pack: CommerceVoipMinutePack) => {
    ...
    try {
      ...
      const redeemed = await redeemVoipMinutePack({
        uid,
        packId: pack.id,
      ...
      if (redeemed.ok) {
        Alert.alert(
          t('sub_voip_pack_success_title'),
          t('sub_voip_pack_success_body', { minutes: String(pack.minutes) }),
        );
        onClose();
      } else {
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        packMinutes: { fontSize: 22, fontWeight: '800', color: shell.ctaAccent },
      ...
      <Pressable style={styles.backdrop} onPress={onClose}>
        <Pressable style={[styles.sheet, { paddingBottom: Math.max(insets.bottom, 16) }]} onPress={() => undefined}>
          <View style={styles.header}>
            <Text style={styles.title}>{t('sub_voip_minutes_section_title')}</Text>
          ...
          >
            <Text style={styles.hint}>{t('sub_voip_minutes_section_hint')}</Text>
            {loading ? (
              ...
              packs.map((pack) => (
                <View key={pack.id} style={[styles.packCard, pack.popular && styles.packPopular]}>
                  <Text style={styles.packMinutes}>
                    {pack.minutes} {t('sub_minutes_label')}
                  </Text>
                  ...
                  <LuxCtaButton
                    ...
                    icon={buyingId === pack.id ? undefined : 'phone-in-talk'}
            ...
            >
              <Text style={styles.tierLinkText}>{t('sub_voip_see_tier_plans')}</Text>
            </TouchableOpacity>
          </ScrollView>
        </Pressable>
      </Pressable>
